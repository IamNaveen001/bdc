...
function Admin() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditing) {
      try {
        await axios.put(`http://localhost:3000/api/donors/${formData._id}`, formData);
        fetchDonors();
        setIsEditing(false);
      } catch (error) {
      ...
      try {
        await axios.post("http://localhost:3000/api/donors", formData);
        fetchDonors();
      } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/api/donors/${id}`);
      fetchDonors();
    } catch (error) {
